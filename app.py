...
calc_min_weight = 16 * (height_in_meters ** 2)

# Calculate Target Weight exactly as it is in fatloss_env.py
calc_target_weight = 22.0 * (height_in_meters ** 2) 
...
# Calculate Ideal Body Fat in kg
target_fat_mass_kg = weight * (calc_target_fat / 100.0) 

# Display in the Sidebar
st.sidebar.subheader("🎯 Optimization Goals")
st.sidebar.info(
    f"**Target Weight:** {calc_target_weight:.1f} kg\n\n"
    f"**Target Fat %:** {calc_target_fat:.1f}%\n\n"
    f"**Target Fat Mass:** {target_fat_mass_kg:.1f} kg\n\n"
    f"**Safe Minimum Weight:** {calc_min_weight:.1f} kg"
)
...
if st.sidebar.button("🚀 Run"):
    ...
    if weight <= calc_target_weight:
        st.warning("⚖️ Your current weight has already reached or is below the target weight!")

    if fat_pct <= calc_target_fat:
        st.warning("🔥 Your current body fat percentage has already reached or is below the target!")

    # If both achieved → stop simulation
    if (weight <= calc_target_weight) and (fat_pct <= calc_target_fat):
        st.success("🎉 You have already achieved your target! No optimization needed.")
        st.stop()
    ...
    st.markdown("---")
    st.write("### ⭐ Cumulative Reward Graph")
    
    # Calculate the cumulative sum of the daily rewards
    df_results['Cumulative_Reward'] = df_results['Reward'].cumsum()
    
    st.line_chart(
        df_results.set_index('Day')['Cumulative_Reward'], 
        x_label="Day", 
        y_label="Total Accumulated Reward"
    )
